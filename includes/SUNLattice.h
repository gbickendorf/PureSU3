...
public:
  ...
  void WilsonLoopRun(const char *filename, int autocorrTime, int N,
...
private:
  ...
  // MultiAlg
  int ThreeCoordinatesToIndex(int x, int y, int z, int timeSlice, int direction,
                              int SliceCount);
  ...
  Complex TwoLinkOp(LatticePosition x, int spaceDir, int seperation, int a,
                    int b, int c, int d);
};
} // namespace SUN
#endif
