...
double Random::GetDouble() { return dist_real01(mt); }
...
matrix2 Random::GetUnitaryMatrix2() {
  ...
  mat.M[0][1] = c2;
  mat.M[1][0] = -conj(c2);
  if (GetDouble() < 0.5) //This actually has consequences!!!
    return mat;
  else
    return mat.dagger();
}
//Calculate random SU3 matrix
...
//Calculate random lattice position
LatticePosition Random::GetLatticePosition() {
  return LatticePosition(
      GetInt(settings.t_LattSize), GetInt(settings.s_LattSize),
      GetInt(settings.s_LattSize), GetInt(settings.s_LattSize), settings);
}
